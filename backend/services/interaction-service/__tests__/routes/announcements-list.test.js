...
describe('公告路由测试 - 获取公告列表', () => {
  ...
  describe('GET /api/interaction/announcements', () => {
    it('应该成功获取公告列表', async () => {
      ...
      // 创建模拟链式调用
      const mockPopulate2 = jest.fn().mockReturnValue(mockAnnouncements);
      ...
      expect(mockPopulate2).toHaveBeenCalledWith('class', 'name grade');
  ...
  describe('GET /api/interaction/announcements/:id', () => {
    it('应该成功获取单个公告', async () => {
      ...
      // 创建模拟链式调用
      const mockPopulate2 = jest.fn().mockReturnValue(mockAnnouncement);
      const mockPopulate1 = jest.fn().mockReturnValue({ populate: mockPopulate2 });
      ...
      expect(response.body).toHaveProperty('content', '测试内容1');
