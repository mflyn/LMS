...
describe('公告路由测试 - 修复版', () => {
  ...
  describe('GET /api/interaction/announcements/:id', () => {
    it('应该成功获取单个公告', async () => {
      ...
      // 由于日期格式在JSON序列化过程中会变成字符串，我们需要单独验证各个字段
      expect(response.body._id).toEqual(mockAnnouncement._id);
      expect(response.body.title).toEqual(mockAnnouncement.title);
      expect(response.body.content).toEqual(mockAnnouncement.content);
