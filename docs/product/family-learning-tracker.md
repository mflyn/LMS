...
- 支持一个家庭管理多个孩子。
- 支持家长在多个孩子之间切换视图。
- 支持创建家长通过 72 小时一次性邀请链接邀请第二家长。
- 两位家长对孩子和日常成长功能同权；当前家庭所有者（初始为创建家长）额外负责邀请、移除和所有权转移。
- 第二家长可以主动退出；当前所有者退出前必须先转移所有权。
- 家长退出或被移除后立即失去家庭访问权限，历史记录和原创建者归属保留。
...
- 展示创建家长和第二家长的关系与所有者状态。
- 创建、复制和撤销第二家长邀请。
- 第二家长退出、创建家长移除成员和转移所有权。
- 明确说明退出或移除不删除孩子历史记录。
...
| `FR-FAM-004` | 第二家长共管 | 12 | 12 | family-enhancement | approved_design | 创建家长的家庭当前只有一位家长，第二家长具有已登录家长账号 | 创建并接受 72 小时一次性邀请 | 最多一位第二家长成功加入；两位家长对孩子档案、任务、记录、错题、附件、能力点、奖励、周报、提醒和家庭基础设置同权；无效、过期、撤销或已使用邀请统一返回 `409 FAMILY_INVITATION_NOT_ACTIVE`；已加入其他家庭返回 `409 PARENT_ALREADY_IN_FAMILY`。 |
| `FR-FAM-005` | 家庭成员治理 | 12 | 12 | family-enhancement | approved_design | 家庭已有两位家长 | 当前所有者移除成员或转移所有权，第二家长主动退出 | 只有所有者可邀请、撤销邀请、移除第二家长和转移所有权；第二家长可退出；所有者退出返回 `409 OWNER_TRANSFER_REQUIRED`；退出或移除后访问立即失效，历史业务记录和操作者归属不删除或匿名化。 |
...
| `FR-MISTAKE-001` | 管理智育错题 | 6 | 6 | mvp | implemented | 家长拥有孩子或孩子本人登录 | 创建、订正或查询错题 | 只接受 academic 维度；记录错因、知识点、订正和复习日期；跨家庭返回 `403`；Task 6 final gate 已覆盖 `TC-T6-MISTAKE-001`-`014`。 |
| `FR-REPORT-001` | 生成成长周报 | 6 | 6 | mvp | implemented | 家长拥有孩子或孩子本人登录 | 查询周一开始的自然周 | 按 5.7 固定公式汇总五育任务、GrowthLog 时长、错题和反馈；历史周不受周末后的取消或补完成影响；必需数据源失败返回 `503 AGGREGATION_UNAVAILABLE`；Task 6 final gate 已覆盖 `TC-T6-REPO-001`-`007` 和 `TC-T6-REPORT-001`-`018`。 |
| `FR-MEDIA-001` | 上传和读取私有图片 | 6 | 6 | mvp | implemented | 家长拥有孩子或孩子本人登录 | 上传头像、任务、错题或成长证据并请求读取 | 媒体上传/读取、孩子头像、GrowthTask 附件、错题 question/answer 媒体引用、gateway signed content 和最终 Task 6 门禁均已覆盖；总追踪见 `family-growth-task6-media-traceability.md`。 |
| `FR-MEDIA-002` | 删除和清理私有图片 | 6 | 6 | mvp | implemented | 家长拥有媒体或孩子拥有本人媒体 | 删除媒体 | 媒体软删除、访问失效、引用保留、30 天清理、错题媒体替换/移除和最终 Task 6 部署门禁均已覆盖；总追踪见 `family-growth-task6-media-traceability.md`。 |
...
| `FR-UI-002` | 孩子简化 Web 入口 | 10 | 10 | mvp | implemented | 孩子具有有效 PIN | 登录、导航、完成任务和退出 | 只展示本人数据和允许动作；家长路由不可达；PIN 失败和过期 token 显示可恢复状态；退出清除 token；Task 10 已通过 `TC-T10-*`、桌面/360px 浏览器验收和 PR #11 远端门禁。 |
| `FR-FLOW-001` | 家庭成长端到端闭环 | 11 | 11 | mvp | implemented | 空白家庭账号 | 执行第一阶段演示流程 | Task 11 真实网关集成与 Chromium 自动化已覆盖 10.3 完整闭环、跨角色切换、图片私有访问、历史周报冻结、提醒去重及星星兑换幂等；门禁证据见 `family-growth-task11-gate.md`。 |
...
| `NFR-DATA-003` | 家长成员关系原子性与审计 | 12 | 12 | family-enhancement | approved_design | 邀请接受、退出、移除或转移所有权 | 并发请求、失败或重放 | Family、User、邀请状态和不可变成员事件在同一 MongoDB 事务提交；家庭始终包含所有者且最多两位唯一家长；并发接受最多一人成功；失败不留下部分关系；邀请明文和摘要不得出现在日志或审计事件。 |
| `NFR-PRIVACY-001` | 未成年人媒体隐私 | 6 | 6 | mvp | implemented | 上传、读取或删除孩子媒体 | 任一媒体操作 | 媒体核心、日志脱敏、孩子头像、GrowthTask 附件、错题媒体隐私、gateway signed content、Task 6 部署检查和最终门禁均已覆盖；总追踪见 `family-growth-task6-media-traceability.md`。 |
...
第一阶段只实现家长 Web 端和孩子简化入口。Task 12 在该 Web 边界内增加第二家长共管；移动端仅做兼容性评估，不作为首轮验收入口。
