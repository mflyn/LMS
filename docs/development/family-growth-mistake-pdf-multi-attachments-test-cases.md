...
| `TC-MPA-MEDIA-003` | Upload zero-page, fifty-one-page, encrypted, password-required, malformed, and invalid cross-reference PDFs. | Stable `PDF_INVALID` or `PDF_PAGE_LIMIT_EXCEEDED`; no residual state. | PDF inspector and route tests |
| `TC-MPA-MEDIA-004` | Upload each approved active/embedded-content rejection fixture. | Stable `PDF_ACTIVE_CONTENT_REJECTED`; rejected bytes never reach storage or scanner. | PDF inspector and pipeline-order tests |
