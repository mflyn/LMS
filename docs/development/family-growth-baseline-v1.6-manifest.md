...
**evidenceBaseCommit:** `1be9f4fd453a3e561802e1605042b556b5276ecc`
...
本清单不记录自身哈希。`evidenceBaseCommit` 是编写本清单时已核实的远端
`main`；`latestFeatureMergeCommit` 是 Task 11 功能合并提交；
`postGateRemediationMergeCommit` 是 Task 1~7 测试质量、配置和基础设施补漏的
后续合并提交。
...
时序，Task G+H 已补齐 35 项需求设计资产追踪和 API 反向设计索引；后续 Task I~J
将继续完善用户指南和旧文档治理。
...
| 证据 | 已通过结果 |
| --- | --- |
| Task 8 frontend gate | 7 suites / 22 tests；生产构建；52 suites / 652 family tests |
| Task 9 frontend gate | 12 suites / 68 tests；生产构建；52 suites / 652 family tests；桌面和 360px 浏览器检查 |
| Task 10 frontend gate | 20 suites / 130 tests；生产构建；52 suites / 652 family tests；桌面和 360px 浏览器检查 |
| Task 11 backend acceptance | 4 suites / 6 tests，连续两次通过 |
| Task 11 frontend gate | 23 suites / 149 tests；生产构建通过 |
| Task 11 Chromium E2E | 4 tests，1 worker，0 retries，通过 |
| Task 11 family regression | 58 suites / 675 tests，通过 |
| Task 1~7 后续质量补漏 | PR [#15](https://github.com/mflyn/LMS/pull/15) 的 Family Regression [29197622721](https://github.com/mflyn/LMS/actions/runs/29197622721) 通过并合并为 `2463f3f0` |

各数值来自对应 gate 在其候选提交上的记录。后续 PR #15 增强了测试质量、配置
和部署验证，因此不会把不同提交上的套件总数误写成同一次运行结果。

## 5. 已知非阻断风险与非 MVP Gate 项

| 风险/事项 | 处理结论 |
| --- | --- |
| 旧学校版遗留测试 | 已与默认家庭成长测试路径隔离；不作为 Task 1~11 MVP gate。保留它们不等于家庭回归允许失败。 |
| CRA 与 Browserslist | 现有 Create React App 工具链会报告 Browserslist 数据过期和部分开发服务器弃用警告；生产构建和当前测试均通过，升级需独立依赖维护任务。 |
| 依赖审计 | 已知 `npm audit` 结果来自既有依赖树；不得在本基线中使用未经评审的强制升级。需独立修订并重新执行回归。 |
| 浏览器覆盖 | Task 11 强制门禁覆盖 Chromium、桌面和 360x800；Safari、Firefox 和原生移动端不在本 MVP gate。 |
...
| 文档补齐 | Task B~H 已纳入当前候选；Task I~J 尚待实施。它们是文档治理工作，不阻断已关闭功能，但 v1.6 正式签署前应评审其对权威文档引用的影响。 |

## 6. 审批与变更控制

| Role | Name | Decision | Date |
| --- | --- | --- | --- |
| Technical reviewer | Codex | READY_FOR_REVIEW | 2026-07-13 |
| Product owner | linmingfeng | PENDING | PENDING |

批准 v1.6 时必须：

1. 将 `status` 更新为 `APPROVED`，记录产品批准人和时间。
2. 确认 `evidenceBaseCommit` 仍包含表中全部 gate 与 merge commit；若主干已改变，
   重新执行差异审计并更新证据提交。
