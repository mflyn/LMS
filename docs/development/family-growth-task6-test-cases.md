...
**Document status:** PROPOSED_FOR_REVIEW  
...
| --- | --- | --- | --- | --- | --- |
| `TC-T6-MEDIA-001` | `FR-MEDIA-001`, `NFR-PRIVACY-001` | route | Parent uploads generated JPEG, PNG, and WebP files with valid purposes. | `201`; sanitized object and active metadata persist; response exposes no storage key or URL. | `familyMedia.test.js` |
...
| `TC-T6-MEDIA-006` | `FR-MEDIA-001`, `NFR-PRIVACY-001` | route | Owner requests access with a fixed clock. | URL expiry is no more than 300 seconds; response is `no-store` and contains no storage key. | `familyMedia.test.js` |
| `TC-T6-MEDIA-007` | `FR-MEDIA-001`, `NFR-PRIVACY-001` | security | Read content with valid, altered, expired, or wrong-media signatures. | Only valid unexpired capability streams exact sanitized bytes with private no-store/nosniff headers. | `familyMedia.test.js` |
| `TC-T6-MEDIA-008` | `FR-MEDIA-001`, `NFR-SEC-001` | security | Family B or sibling A2 requests access for A1 media. | `403 CHILD_ACCESS_DENIED`; no URL or metadata disclosed. | `familyMedia.test.js` |
| `TC-T6-MEDIA-009` | `FR-MEDIA-002` | route | Delete active media twice, then request access/content and prepare a new reference. | Both deletes return `204`; media is immediately inaccessible and unreferenceable. | `familyMedia.test.js` |
| `TC-T6-MEDIA-010` | `FR-MEDIA-002`, `NFR-PRIVACY-001` | retention | Evaluate deleted objects at 29 days, 30 days with a reference, and 30 days without references. | Only the last object is cleanup eligible; repeated cleanup is harmless. | `privateMediaStore.test.js` |
...
| `TC-T6-MISTAKE-004` | `FR-MISTAKE-001` | authorization | Child A1 creates or patches only child-answer, correction/review/mastery, and explanation fields. | Allowed mutation succeeds and parent-owned fields remain unchanged. | `familyMistakes.test.js` |
| `TC-T6-MISTAKE-005` | `FR-MISTAKE-001` | authorization | Child writes question, correct answer, reason, parent note, knowledge point, ownership, or audit fields. | `403 FIELD_ACCESS_DENIED`; stored record unchanged. | `familyMistakes.test.js` |
...
| `TC-T6-MISTAKE-007` | `FR-MISTAKE-001` | route | List A1 mistakes by subject, reason, corrected, reviewed, mastered, and reminder range with pagination. | Only scoped matches return with correct page metadata and stable ordering. | `familyMistakes.test.js` |
| `TC-T6-MISTAKE-008` | `FR-MISTAKE-001` | route | Parent and child patch each documented allowed field combination. | `200`; only supplied authorized fields change and audit fields update. | `familyMistakes.test.js` |
...
| `TC-T6-REPO-003` | `FR-REPORT-001` | repository | Read log, knowledge-point, and mistake projections. | Lean projected values return without Mongoose documents or private text fields not needed by formulas. | `familyReadRepository.test.js` |
...
| `TC-T6-REPORT-003` | `FR-REPORT-001` | aggregation | Create 30 planned tasks with 24 completed before cutoff across five dimensions. | Completion rate is 80 and each dimension's planned/completed count is correct. | `weeklyReports.test.js` |
...
| `TC-T6-REPORT-009` | `FR-REPORT-001` | aggregation | Create due unmastered mistakes and duplicate needs-review knowledge points. | Review points are unioned, deduplicated, and stable-sorted; mastered/future items are absent. | `weeklyReports.test.js` |
...
| `TC-T6-REPORT-013` | `FR-REPORT-001` | current week | Generate current week, add source data, and request again. | Non-frozen snapshot recomputes and reflects new source data. | `weeklyReports.test.js` |
| `TC-T6-REPORT-014` | `FR-REPORT-001` | concurrency | Issue concurrent first requests for one ended week. | Unique key yields one frozen snapshot; all successful responses return the winner. | `weeklyReports.test.js` |
...
| `TC-T6-REG-001` | Task 6 gate | startup | Import resource/analytics app modules under Jest and start explicit test servers. | Imports open no DB/port/process handler; tests control one Mongo and close all handles. | `task6Startup.test.js` |
| `TC-T6-REG-002` | `NFR-PRIVACY-001` | deployment | Validate Compose/Kubernetes volume and external secret wiring plus static-route configuration. | Private persistent volume exists, secrets are external, media root is never a static directory. | `task6Deployment.test.js` |
...
| Requirement | Cases |
| --- | --- |
| `FR-MEDIA-001` | `TC-T6-MEDIA-001`-`008`, `011`-`014`; `TC-T6-MISTAKE-009`-`012`; `TC-T6-GW-003` |
| `FR-MEDIA-002` | `TC-T6-MEDIA-009`-`010`; `TC-T6-MISTAKE-011` |
| `FR-MISTAKE-001` | `TC-T6-MISTAKE-001`-`014` |
| `FR-REPORT-001` | `TC-T6-REPO-001`-`006`; `TC-T6-REPORT-001`-`018` |
| `NFR-PRIVACY-001` | `TC-T6-MEDIA-001`, `003`, `006`-`010`, `015`; `TC-T6-REG-002` |
| `NFR-SEC-001` | Cross-family, sibling, internal-command, repository-scope, gateway, and deployment cases above |
| `NFR-DATA-001` | Reference idempotency, report snapshot, concurrency, and regression cases above |
| `NFR-TIME-001` | `TC-T6-MISTAKE-002`; `TC-T6-REPORT-001`, `004`-`005`, `012`-`013` |

The catalog contains 62 distinct case IDs. Formal test review must confirm uniqueness, requirement coverage, executable ownership, and consistency with the detailed design before implementation begins.
