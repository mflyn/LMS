...
`POST /api/auth/logout` 无请求体，重复调用均返回 `204`。JWT 第一阶段不维护服务端黑名单，客户端必须删除对应家长或孩子 token；PIN 重置仍通过 `tokenVersion` 强制撤销孩子旧 token。

`PATCH /api/families/:familyId` 只接受 `name`、`timezone`。`timezone` 必须是有效 IANA 名称；客户端不能修改 owner/member/childIds。成功返回更新后的 family；跨家庭 `403 CHILD_ACCESS_DENIED`，不存在 `404 RESOURCE_NOT_FOUND`。
...
`PATCH /api/children/:childId` 只允许家长修改 `name`、`avatarMediaId`、`grade`、`school`、`curriculumVersion`、`interests`、`weakSubjects`、`sportPreferences`、`artInterests`、`laborHabits`、`moralGoals`。`avatarMediaId` 必须是本家庭 `purpose=avatar` 的 active 媒体；不能修改 familyId、role、PIN 哈希或 tokenVersion。成功返回完整 child profile。
...
`attachmentMediaIds` 可选；每项必须是本家庭 active、purpose=`task_attachment` 的媒体。FR-MEDIA-001 上线前该字段返回 `400 MEDIA_NOT_ENABLED`，不得接受任意 URL 或未验证元数据。
...
`PATCH /api/growth-tasks/:taskId` 仅家长可用，只接受创建接口中的可编辑计划字段：`dimension`、`area`、`subject`、`title`、`taskType`、`description`、`dueDate`、`estimatedMinutes`、`targetAmount`、`unit`、`priority`、`attachmentMediaIds`。只允许 `pending` 状态编辑；终态返回 `409 TASK_STATE_CONFLICT`。附件必须是本家庭 active 且 purpose 为 `task_attachment` 的媒体。
...
固定公式：计划分母为 dueDate 在周内、周末前已创建且未在周末前取消的任务；完成分子还要求 completedAt 不晚于周末。分母为 0 时 `taskCompletionRate=null`。时长只汇总周内 GrowthLog.durationMinutes，不叠加任务 actualMinutes。取消或补完成发生在周末之后时，不改写历史周。
...
`POST /api/media` 使用 `multipart/form-data`，字段为 `file`、`purpose` 和可选 `childId`。允许 `image/jpeg|image/png|image/webp`，最大 10 MiB；服务端以文件签名而非扩展名判断类型并移除 EXIF。孩子 token 的 childId 强制取认证身份，且 purpose 只允许 `task_completion|mistake_question|mistake_answer|growth_evidence`。
...
`GET /api/media/:mediaId/access` 在校验 familyId、childId 和 `status=active` 后返回最长 300 秒的授权 URL 与 `expiresAt`。响应禁止缓存：`Cache-Control: no-store`。不存在返回 `404 RESOURCE_NOT_FOUND`，跨家庭或兄弟姐妹访问返回 `403 CHILD_ACCESS_DENIED`。

### 8.3 删除图片

`DELETE /api/media/:mediaId` 将状态置为 deleted 并记录 deletedAt，重复调用幂等返回 `204`。删除后授权读取与新引用均返回 `404 RESOURCE_NOT_FOUND`；解除全部业务引用 30 天后才能物理清理对象。
...
`GET /api/notifications/settings` 返回当前家庭唯一设置。首次读取尚无记录时返回默认值，不通过读取创建数据库副作用。家长和孩子均可读取，孩子不能修改。

`PATCH /api/notifications/settings` 仅家长可用，接受各提醒布尔开关、`weeklyReportDay`（ISO 1 至 7）以及成对的 `quietHoursStart/quietHoursEnd`（`HH:mm`）。成功返回完整设置；未知字段或无效时间返回 `400 VALIDATION_ERROR`。

`GET /api/notifications/family` 的 today、到期日、周报日和安静时段均按家庭时区计算。结果按 `type + childId + LocalDate + sourceId` 去重并按 `priority desc, dueDate asc, type asc, sourceId asc` 稳定排序；关闭的类别不得返回。部分源失败仍返回 `200`，同时设置 `meta.partial=true` 和 `meta.unavailableSources`。
