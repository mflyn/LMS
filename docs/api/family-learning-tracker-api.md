...
`PATCH /api/children/:childId` 只允许家长修改 `name`、`avatarMediaId`、`grade`、`school`、`curriculumVersion`、`interests`、`weakSubjects`、`sportPreferences`、`artInterests`、`laborHabits`、`moralGoals`。`avatarMediaId` 必须是本家庭、同一 childId、`purpose=avatar` 的 active 媒体；不能修改 familyId、role、PIN 哈希或 tokenVersion。服务端在返回可用 avatarMediaId 前完成引用绑定；提交已保存但绑定暂未完成时返回 `503 MEDIA_REFERENCE_PENDING` 和 `details.resourceId`，相同 PATCH 可安全恢复。成功返回完整 child profile。
