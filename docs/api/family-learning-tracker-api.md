...
家长可提交全部已记录的业务字段。孩子创建错题时必须提交 `subject` 和 `reason`，并可同时提交 `childAnswerMediaId`、`corrected`、`reviewed`、`mastered`、`childExplanation`；创建后孩子不能修改 `subject` 或 `reason`。孩子 Web 快速录入只提交 `subject`、`reason` 和可选 `childExplanation`，`familyId` 与 `childId` 必须由已验证的孩子 token 派生，不接受页面选择或覆盖。
