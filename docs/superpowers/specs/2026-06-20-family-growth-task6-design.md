...
- `connectDatabase(config)` establishes and verifies the MongoDB connection.
...
| --- | --- | --- |
| `familyId`, `childId` | ObjectId | required; all access predicates start with both |
...
`MediaReference` is a resource-service internal collection that prevents an unbounded references array in `MediaAsset`. Each row contains `familyId`, `childId`, `mediaId`, `resourceType`, `resourceId`, `field`, `operationId`, `state=prepared|bound`, and `leaseExpiresAt`. The unique key is:

```text
{ mediaId: 1, resourceType: 1, resourceId: 1, field: 1 }
```

The effective reference set of a media asset is the active `MediaReference` rows owned by resource-service. No other service reads or writes this collection directly.
...
1. Resolve `familyId` and effective `childId` from the authenticated identity.
2. Enforce the actor/purpose allowlist. Child uploads permit only `task_completion`, `mistake_question`, `mistake_answer`, and `growth_evidence`.
...
`GET /api/media/:mediaId/access` first authorizes the authenticated family and child against an active asset. It returns a URL valid for at most 300 seconds and `expiresAt`; the response has `Cache-Control: no-store`.

The URL points to `GET /api/media/:mediaId/content` and contains an expiry, random nonce, and HMAC signature over the normalized path, media ID, and expiry. `MEDIA_URL_SIGNING_SECRET` is independent of JWT and service credentials, has at least 32 characters, and signatures are compared in constant time.
...
`DELETE /api/media/:mediaId` is idempotent. The first call atomically changes `active` to `deleted`, sets `deletedAt`, and invalidates all prepared or bound references. Repeated calls return `204`.
...
- status is `deleted`;
- `deletedAt` is at least 30 days old;
- no active `MediaReference` remains.
...
Analytics uses one preallocated mistake ObjectId and one operation ID for a mutation:
...
2. Analytics saves the mistake with `mediaReferenceState=pending` and the operation ID.
3. `POST /api/internal/media/references/commit` changes exactly those prepared rows to `bound`; analytics changes its state to `bound`.
...
All commands are idempotent by operation ID and reference identity. If mistake persistence fails, analytics performs best-effort unbind; an expired prepared lease is also reclaimable. If commit fails after persistence, analytics returns `503 MEDIA_REFERENCE_PENDING` with `details.mistakeId`. A later `GET` or `PATCH` of that mistake first resumes the same commit. Pending media fields are not returned as usable references until binding converges.
...
Every read and mutation predicate contains `familyId + childId`. A parent may create and update all documented business fields. A child may create and update only `childAnswerMediaId`, `corrected`, `reviewed`, `mastered`, and `childExplanation`; parent-owned answer, reason, question, and note fields return `403 FIELD_ACCESS_DENIED`. Ownership, dimension, audit, and binding fields are never client writable.
...
The implementation returns only fields required by report formulas, uses lean reads, and rejects missing family/child filters. The default timeout is 2000 ms and can be lowered in tests. It does not return Mongoose documents and has no mutation API.
...
The current week is recomputed from source projections on every request in Task 6; it may replace its non-frozen snapshot. An ended week is computed once and inserted with `frozen=true`. A duplicate-key race reads and returns the winner's snapshot. No route can patch statistics or unfreeze a report.
...
- Completion rate: `completed / planned * 100`, rounded according to the API contract; when planned is zero return `null`.
...
- Review points: union unmastered mistakes due by cutoff and knowledge points with `masteryLevel=needs_review`; deduplicate and stable-sort by dimension, subject/area, then name.
...
Compose and Kubernetes mount a persistent resource-service volume and receive both secrets through the existing external `family-growth-secrets` process. No Secret manifest or rendered credential is committed. Resource and analytics startup fail when Task 6 routes are enabled but required configuration is invalid.
...
- targeted resource, analytics, common repository, and gateway suites pass;
...
2. Add failing media tests, then private store, models, routes, reference commands, deployment, and gateway wiring.
3. Add failing mistake tests, then model, role authorization, media binding recovery, and routes.
4. Add failing repository/report tests, then projections, aggregation, snapshot freezing, feedback, and routes.
