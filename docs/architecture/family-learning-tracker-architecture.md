...
- Task 12 回滚只停用邀请和成员治理路由及页面；现有服务已经按 `ownerParentId` 或
  `memberParentIds` 授权，因此保留已经形成的两家长关系和历史事件，不执行破坏性回退。
- 发布前运行成员关系修复命令，确定性修复缺失 owner、重复 member、同家庭
  `User.familyId` 和 `User.children` 投影；一个家长同时出现在多个家庭时只报告冲突，
  不自动猜测归属。
...
脱敏 POST 请求体提交 token。邀请明文不写入浏览器持久存储。
